...
loki_decay_finder = appMgr.toolsvc().create('LoKi::Decay',
                                            interface="Decays::IDecay")
# Old style decay descriptors
old_decay_finder = appMgr.toolsvc().create("DecayFinder",
                                           interface="IDecayFinder")

# works
# decay_desc = ("[[B0]cc -> "
#               "(^D- => "
#               "{^K- ^K+ ^pi-, ^K- ^pi+ ^pi-,^pi+ ^pi- ^pi-, ^K- ^K- ^pi+}) "
#               "^K-]cc")
# doesn't work
decay_desc = ("[[B0]cc -> "
              "(^D- => {^K- ^K+ ^pi-, ^K- ^pi+ ^pi-,^pi+ ^pi- ^pi-}) ^K-]cc")
